			 ...
			 if(rdata.list.length>0){
				   output += '<li class="comment_tab_item ' +  red1 + '" >'
					...
					$(rdata.list).each(function(){
						...
						output += '<li id="' + this.event_board_num + '" class="CommentItem' + comment_reply + '">'
						...
						if(lev<2){ //답글쓰기는 답글의 답글까지만 사용하도록 합니다.
						   	  output += '  <a href="javascript:replyform(' + this.event_board_num+',' 
						   	         + lev + ',' + this.event_re_seq +',' 
						   	         + this.event_re_ref +')"  class="comment_info_button">답글쓰기</a>'
						      }
						output += '   </div>' //comment_info_box;
							   
						//글쓴이가 로그인한 경우 나타나는 더보기입니다.
                        //수정과 삭제 기능이 있습니다.							
					    if($("#id").val()==this.id||$("#id").val()=='admin'){  
						 output +=  '<div class="comment_tool">'
							   + '    <div title="더보기" class="comment_tool_button">'
							   + '       <div>&#46;&#46;&#46;</div>' 
							   + '    </div>'
							   + '    <div id="commentItem' +  this.event_board_num + '"  class="LayerMore">' //스타일에서 display:none; 설정함
							   + '     <ul class="layer_list">'							   
							   + '      <li class="layer_item">'
							   + '       <a href="javascript:updateForm(' + this.event_board_num + ')"'
							   + '          class="layer_button">수정</a>&nbsp;&nbsp;'
							   + '       <a href="javascript:del(' + this.event_board_num + ')"'
					...
					if(page!=1)
					 output+='<li class="page-item">'
						   +'<a href="javascript:getList('+ (rdata.page-1)+ ','+ rdata.state+')"' 
						   +' class="page-link">이전&nbsp;</a></li>'
					 for(var i=rdata.startpage;i<=rdata.endpage;i++){
						 if(i==rdata.page){
							 output+='<li class="page-item ">'
								 +'<a class="page-link gray">'+i+'</a></li>'
						 }else{
							 output+='<li class="page-item ">'
								 +'<a href="javascript:getList('+ i + ','+ rdata.state+')"' 
