			...
			success:function(rdata){
				...
				if(rdata.listcount>0){
					$(rdata.list).each(function(index,item){
						...
						output+= '<a href="productDetail?code=' + item.code 
							  + '&productname=' + item.productname+'&description=' 
							  + item.description+'&perpoint=' + item.perpoint+'&img='
							  + item.img+ '&quantity='+item.quantity+'"  class="img-prod"><img class="img-fluid"'
						      + 'src="../upload' + item.img + '" alt="Colorlib Template">';
						...
						output+= '<a href="productDetail?code=' + item.code 
						      +  '&productname=' + item.productname+'&description=' 
						      +  item.description+'&perpoint=' + item.perpoint+'&img='
						      +  item.img +'&quantity='+item.quantity
						      +  '">' + item.productname.replace(/</g,"&lt;").replace(/>/g,"&gt;") +'</a>';
						output+= '</h3>';
						output+= '<div class="d-flex">';
						output+= '<div class="pricing">';
						output+= '<p class="price">';
						output+= '<span id="price">'+ item.perpoint+'P</span>';
						output+= '</p></div></div>';
						output+= '<div class="bottom-area d-flex px-3">';
						output+= '<div class="m-auto d-flex">';
//						
						output+= '<a href="javascript:addCart('+item.code+',';
						output+=  +item.perpoint+')"'
						     
						       +' class="add-to-cart d-flex justify-content-center align-items-center text-center">'
						output+= '<span><i class="ion-ios-cart"></i></span></a>'
					    output+= '<a href="buynow?quantity=1&code=' + item.code + '&productname='
					              +item.productname+'&perpoint=' + item.perpoint+ '"  class="buy-now d-flex justify-content-center align-items-center mx-1">'	
					...
					output=""
					   ...
					   if(rdata.page<=1){
						   ...
						   output+=' <li><a href="javascript:getList('+ (rdata.page - 1) + ')" class="lin">&lt;</a></li>';
					   }
					   for(var i=rdata.startpage;i<=rdata.endpage;i++){
						   if(i==rdata.page){
							   output+='<li><a class="active">'+i + '</a></li>';
						   }else{
							   output+='<li><a href="javascript:getList('+i + ')" class="lin">' + i + '</a></li>';
						   }
					   }
