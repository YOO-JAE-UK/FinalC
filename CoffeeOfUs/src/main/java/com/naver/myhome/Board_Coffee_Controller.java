...
import com.naver.myhome.service.Board_Coffee_Service;
import com.naver.myhome.service.Comment_Coffee_Service;


@Controller
@RequestMapping(value = "/board_coffee")
public class Board_Coffee_Controller {
	
	private static final Logger logger 
	= LoggerFactory.getLogger(Board_Coffee_Controller.class);
	
	@Autowired
	private Board_Coffee_Service Board_Coffee_Service;
	...
	public String board_wrtie() {
		return "board_coffee/board_write";
	}
	...
	public ModelAndView boardList(
		...
		int listcount = Board_Coffee_Service.getListCount(); //총 리스트 수를 받아옴
		...
		List<Board_Coffee> boardlist = Board_Coffee_Service.getBoardList(page, limit);// 리스트를 받아옴
				...
				mv.setViewName("board_coffee/board_list");
	...
	public ModelAndView BoardReplyAction(Board_Coffee board, ModelAndView mv,
		...
		if(result == 0) {
			...
			mv.setViewName("redirect:detail?num="+board.getCOFFEE_NUM());
		}
	...
	public ModelAndView BoardModifyView(int num,
		...
		Board_Coffee boarddata = Board_Coffee_Service.getDetail(num);
	...
	public String boardModifyAction(Board_Coffee boarddata, String before_file,
			String check, Model mv, HttpServletRequest request,
			RedirectAttributes rattr
			) throws Exception{
		boolean usercheck =
				Board_Coffee_Service.isBoardWriter(boarddata.getCOFFEE_NUM(), boarddata.getCOFFEE_PASS());
		String url="";
		//비밀번호가 다른 경우
		if(usercheck == false) {
			rattr.addFlashAttribute("result", "passFail");
			rattr.addAttribute("num",boarddata.getCOFFEE_NUM());
		...
		if(check != null && !check.equals("")) {//기존파일 그대로 사용하는 경우입니다.
			...
			boarddata.setCOFFEE_ORIGINAL(check);
			...
			String fileName = uploadfile.getOriginalFilename();//원래 파일명
			boarddata.setCOFFEE_ORIGINAL(fileName);
			...
			//바뀐 파일명으로 저장
			boarddata.setCOFFEE_FILE(fileDBName);
			}else { //uploadfile.isEmpty() 인경우 - 파일을 선택하지 않은 경우
				logger.info("선택 파일 없습니다.");
				//<input type="hidden" name="BOARD_FILE" value=${boarddata.BOARD_FILE}">
				//위 태그에 값이 있다면 ""로 값을 변경합니다.
				boarddata.setCOFFEE_FILE(""); //""로 초기화 합니다.
				boarddata.setCOFFEE_ORIGINAL("");//""로 초기화 합니다.
			}//else end
		}//else end	
		
		//DAO 에서 수정 메서드 호출하여 수정합니다.
		int result = Board_Coffee_Service.boardModify(boarddata);
		//수정에 실패한 경우
		if(result ==0) {
			...
			if(!before_file.equals("") && !before_file.equals(boarddata.getCOFFEE_FILE())) {
				Board_Coffee_Service.insert_deleteFile(before_file);
			}
		}
		return url;
	}
	...
	public String BoardDeleteAction(String BOARD_PASS, int num,
		...
		boolean usercheck = Board_Coffee_Service.isBoardWriter(num, BOARD_PASS);
		...
		int result = Board_Coffee_Service.boardDelete(num);
