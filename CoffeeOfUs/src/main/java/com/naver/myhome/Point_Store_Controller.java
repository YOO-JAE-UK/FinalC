...
public class Point_Store_Controller {
	...
	= LoggerFactory.getLogger(Point_Store_Controller.class);
	...
	public Map<String,Object> getList(
		...
		int listcount=point_store_service.getListCount();
		int limit= 8;
		...
		List<Product> list= point_store_service.getList(page,limit);
		...
		public String addtoCart(HttpSession session,Cart cart) {
			...
			String st="<script type='text/javascript' charset='utf-8'>";
			       st+="alert('Success!!!');"; 
			       st+="history.back()";
			        st+="</script>";
			return st;
	  ...
	  public Product getProductInfo(int code) {
		...
		public Map<String,Integer> Keyup(
			Cart cart) {
	...
	public String finalPay(HttpSession session,RecieveAddress addressinfo,Member member,
			Product product,@RequestParam(value="check") int check,
